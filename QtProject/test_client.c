...
#ifdef KAISER_BESSEL
  // change brightness
  #define TAPS 51   // size of filter window
  float coef[TAPS]; // coefficients
  int g2=51;   // scaling
  int h2=0; // offset
#endif	
...
void
filter_input(jack_default_audio_sample_t out[1],jack_nframes_t delay_index)
{
	...
	for (i=0; i < TAPS; i++)  // loop over all samples of audio stream
	{
		sum += coef[i]*delay_line[delay_index];
...
get_coef()
{
	coef_file = fopen("/home/pi/Qt_Projekt/test/out_coef.txt","r");
	if(coef_file == NULL)
	{
		exit(EXIT_FAILURE);
	}
...
process (jack_nframes_t nframes, void *arg)
{
	...
	for (k=0; k<nframes; k++) { 		
		...
		filter_input(result, delay_index);
